...
static ssize_t nt_rpmh_stats_show(struct kobject *obj, struct kobj_attribute *attr, char *buf) 
{
	...
	for (i = 0; i < drv->config->num_records; i++) {
		stat.stat_type = le32_to_cpu(readl_relaxed(reg + stats_offset + STAT_TYPE_OFFSET));
		stat.count = le32_to_cpu(readl_relaxed(reg + stats_offset + COUNT_OFFSET));
		stat.last_entered_at = le64_to_cpu(readq(reg + stats_offset + LAST_ENTERED_AT_OFFSET));
		stat.last_exited_at = le64_to_cpu(readq(reg + stats_offset + LAST_EXITED_AT_OFFSET));
		stat.accumulated = le64_to_cpu(readq(reg + stats_offset + ACCUMULATED_OFFSET));
		...
		if(drv->config->appended_stats_avail) {
			app_stat.client_votes = le32_to_cpu(readl_relaxed(reg + stats_offset + CLIENT_VOTES_OFFSET));
...
static int qcom_stats_probe(struct platform_device *pdev)
{
	...
	int ret;
#ifdef CONFIG_NOTHING_POWERINFO_RPMH
	struct soc_sleep_stats_data *drvs;
	pr_err("lifan1111");
	drvs = devm_kzalloc(&pdev->dev, sizeof(*drvs), GFP_KERNEL);
	if(!drvs)
		return -ENOMEM;
#endif
...
#ifdef CONFIG_NOTHING_POWERINFO_RPMH
	soc_sleep_stats_create_sysfs(pdev, drvs);
	drv_backup = drvs;
#endif
