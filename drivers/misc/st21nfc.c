...
/*#BELL-423,zhanghao,20250714,NFC,begin*/
#define HW_ST21L 0x07
#define HW_ST54L 0x06
/*#BELL-423,zhanghao,20250714,NFC,end*/
...
static int st21nfc_ping(struct st21nfc_device *st21nfc_dev)
{
	...
	while ((loops-- > 0) && gpiod_get_value(st21nfc_dev->gpiod_irq)) {
		...
		else
		{
			...
			/*#BELL-423,zhanghao,20250806,NFC,begin*/
			else if ((st21nfc_dev->buffer[2]== 0x41) && (st21nfc_dev->buffer[3] == 0xA2)) {
        		pr_info("-> Loader V3 Mode NCI_CORE_RESET_NTF received after HW Reset");
				// check the Product Identification Number
				// 54L: 75 02
				// 21L: 98 02
				if ((st21nfc_dev->buffer[24] == 0x98) && (st21nfc_dev->buffer[25] == 0x02)) {
				nfc_hw_ver = HW_ST21L;
				} else if ((st21nfc_dev->buffer[24] == 0x75) && (st21nfc_dev->buffer[25] == 0x02)){
				nfc_hw_ver = HW_ST54L;
				}
			}
			/*#BELL-423,zhanghao,20250806,NFC,end*/
				...
				struct device_attribute *attr, char *buf)
{
	if (nfc_hw_ver == HW_ST21)
		...
		return scnprintf(buf, PAGE_SIZE, "%s\n", "ST54");
	/*#BELL-423,zhanghao,20250714,NFC,begin*/
	else if (nfc_hw_ver == HW_ST21L)
		return scnprintf(buf, PAGE_SIZE, "%s\n", "ST21NFCL");
	else if (nfc_hw_ver == HW_ST54L)
		return scnprintf(buf, PAGE_SIZE, "%s\n", "ST54L");
	/*#BELL-423,zhanghao,20250714,NFC,begin*/
	else
		return scnprintf(buf, PAGE_SIZE, "%s\n", "ERROR");
