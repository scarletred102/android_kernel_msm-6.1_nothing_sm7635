...
#define GPIO_DEV_NAME  "gpio,board-id,Frogger"
#ifdef CONFIG_OF
	...
	for (i = 0; i < size; i++) {
		if (gpio_is_valid(gpios[i].gpio)) {
			...
			pr_err("%s:%d gpios[%d].gpio = %d, gpios[%d].%s.gpio_value = %d", __func__, __LINE__,
					...
					buffer[i]=gpio_get_value(gpios[i].gpio) ? '1' : '0';
		}
		else {
			buffer[i] = '0';
                }
...
static int board_id_probe(struct platform_device *pdev)
{
	...
	switch(id_value)
	{
		...
		case (7) :
			...
			break;
		case (15) :
			board_id_version = IDX_EVT;
			break;
		case (19) :
			board_id_version = IDX_DVT;
			break;
		case (23) :
			board_id_version = IDX_PVT;
			break;
		case (5) :
			board_id_version = IDX_T0_Japan;
			break;
		case (9) :
			board_id_version = IDX_EVT_Japan;
			break;
		case (21) :
                        board_id_version = IDX_POWER_BOARD_Japan;
                        break;
		case (13) :
			board_id_version = IDX_DVT_Japan;
			break;
		case (17) :
                        board_id_version = IDX_PVT_Japan;
                        break;
		case (6) :
			board_id_version =  IDX_T0_India;
			break;
		case (10) :
			board_id_version =  IDX_EVT_India;
			break;
		case (14) :
			board_id_version = IDX_DVT_India;
			break;
		case (18) :
			board_id_version =  IDX_PVT_India;
			break;
		default:
