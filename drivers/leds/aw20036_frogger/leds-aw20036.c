...
#define AW20036_DRIVER_VERSION "v1.6.0"
...
#define VALID_CHANNEL 6
#define HW_ALL_CHANNEL 7

#define VIDEO_LIGHT_REG 26 //Video indicator light register

/*add for Frogger*/
#define LED0_REG_FADE 0x00
#define LED1_REG_FADE 0x01
#define LED2_REG_FADE 0x0c
#define LED3_REG_FADE 0x0d
#define LED4_REG_FADE 0x18
#define LED5_REG_FADE 0x19
#define LED6_REG_FADE 0x1a
...
static unsigned char video_indicator_val = 0;
...
#define AW20036_BLACK_IMAX AW20036_IMAX_60mA
#define AW20036_WHITE_IMAX AW20036_IMAX_60mA
				...
				unsigned char reg_addr, unsigned char *reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			LOG_ERR("%s: i2c_read cnt=%d error=%d\n", __func__, cnt,
				ret);
		} else {
			*reg_data = ret;
			break;
...
static void aw20036_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	for (i = 0; i < cont->size; i += 2) {
		...
		aw20036_i2c_write(aw20036, *(cont->data + i),
				  *(cont->data + i + 1));
		LOG_INFO("%s: addr:0x%02x, data:0x%02x\n", __func__,
	...
	LOG_INFO("%s: cfg update complete\n", __func__);
}
...
static int aw20036_cfg_update(struct aw20036 *aw20036)
{
	...
	if (aw20036->effect < (sizeof(aw20036_cfg_name) / AW20036_CFG_NAME_MAX)) {
		LOG_INFO("%s: cfg name=%s\n", __func__,
			aw20036_cfg_name[aw20036->effect]);
	} else {
		LOG_ERR("%s: effect 0x%02x over s value\n", __func__,
			aw20036->effect);
		return (-1);
	}
...
{
	LOG_INFO("%s: imax %d\n", __func__, aw20036->imax);

	video_indicator_val = 0;
...
static int aw20036_read_chipid(struct aw20036 *aw20036)
{
	...
	while (cnt++ < AW_READ_CHIPID_RETRIES) {
		...
		if (reg_val == AW20036_CHIPID) {
			LOG_INFO("This Chip is  AW20036 REG_ID: 0x%x\n",
				reg_val);
			return 0;
		} else if (ret < 0) {
			dev_err(aw20036->dev,
			...
			LOG_INFO("This Chip read register REG_ID: 0x%x\n",
				reg_val);
		}
					...
					const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
		...
		if (databuf[0] == VIDEO_LIGHT_REG) {
			aw20036_i2c_write(aw20036, databuf[0], databuf[1]);
			video_indicator_val = databuf[1];
			LOG_INFO("%s: video %d\n", __func__, video_indicator_val);
		} else {
			aw20036_i2c_write(aw20036, databuf[0], databuf[1]);
		}
	}
						  ...
						  const char *buf, size_t len)
{
	...
	for (i = 0; i < ALL_CHANNEL; i++) {
		if ((i == LED0_REG_FADE) || (i == LED1_REG_FADE) || (i == LED2_REG_FADE) ||  (i == LED3_REG_FADE) \
			|| (i == LED4_REG_FADE) || (i == LED5_REG_FADE))
			data[i] = val;
		else
			data[i] = 0;
	}
						...
						const char *buf, size_t len)
{
	...
	for (i = 0; i < ALL_CHANNEL; i++) {
		if ((i == LED0_REG_FADE) || (i == LED1_REG_FADE) || (i == LED2_REG_FADE) ||  (i == LED3_REG_FADE) \
			|| (i == LED4_REG_FADE) || (i == LED5_REG_FADE) || (i == LED6_REG_FADE ))
			data[i] = val;
		else
			data[i] = 0;
	}
						...
						const char *buf, size_t len)
{
	...
	if (frame_num == ALL_CHANNEL) {
		...
		int mapping_channels[VALID_CHANNEL] = {0, 1, 12, 13, 24, 25};
		for (i = 0; i < VALID_CHANNEL; i++) {
			num = mapping_channels[i];
			brightness[num] = frame_brightness[i];
		}
		if (video_indicator_val > 0)
			brightness[VIDEO_LIGHT_REG] = video_indicator_val;
	} else if (frame_num == HW_ALL_CHANNEL) {
		int mapping_channels[HW_ALL_CHANNEL] = {0, 1, 12, 13, 24, 25, 26};
		for (i = 0; i < HW_ALL_CHANNEL; i++) {
			num = mapping_channels[i];
			brightness[num] = frame_brightness[i];
		}
	}
						...
						const char *buf, size_t len)
{
	...
	strncpy(dev_color, buf, 5);
	if (strncmp(dev_color, "BLACK", 5) == 0) {
		LOG_INFO("%s: device color is black\n", __func__);
		aw20036->imax = AW20036_BLACK_IMAX;
	} else {
		aw20036->imax = AW20036_WHITE_IMAX;
	}
	aw20036_imax_cfg(aw20036, aw20036->imax);
	aw20036_rgbcolor_config(aw20036);
...
//for cit
int factory_led0=0, factory_led1=0, factory_led2=0, factory_led3=0, factory_led4 =0,factory_led5=0, factory_led6 =0,factory_led7 =0, factory_led8 =0;
static ssize_t aw20036_factory_test_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	LOG_INFO("%s\n", __func__);

	return sprintf(buf, "%d %d %d %d %d %d %d %d %d\n", factory_led0, factory_led1, factory_led2, factory_led3, factory_led4,factory_led5,factory_led6,factory_led7, factory_led8);
}

static ssize_t aw20036_lamps_flowing_switch(struct aw20036 *aw20036, int lamps_brightness)
{
	int ret = 0;
	LOG_INFO("%s\n", __func__);

	//led0
	ret = aw20036_i2c_write(aw20036, LED0_REG_FADE, lamps_brightness);
	if (ret < 0) {
		goto end;
	}
	...
	//led1
	ret = aw20036_i2c_write(aw20036, LED1_REG_FADE, lamps_brightness);
	if (ret < 0) {
		goto end;
	}
	...
	//led2
	ret = aw20036_i2c_write(aw20036, LED2_REG_FADE, lamps_brightness);
	if(ret < 0){
		goto end;
	}
	...
	//led3
	ret = aw20036_i2c_write(aw20036, LED3_REG_FADE, lamps_brightness);
	if (ret < 0) {
		goto end;
	}
	...
	ret = aw20036_i2c_write(aw20036, LED4_REG_FADE, lamps_brightness);
	if (ret < 0) {
		goto end;
	}
	...
	ret = aw20036_i2c_write(aw20036, LED5_REG_FADE, lamps_brightness);
	if (ret < 0) {
		goto end;
	}
	...
	ret = aw20036_i2c_write(aw20036, LED6_REG_FADE, lamps_brightness);
	if (ret < 0) {
		goto end;
	}
...
static void aw20036_lamps_flowing_work(struct work_struct *work)
{
	...
	int ret = 0;
	int led_off = 0;
	int brightness = factory_led7;
	int status = factory_led8;
	...
	if (status) {
		ret = aw20036_lamps_flowing_switch(aw20036, brightness);
		if (ret < 0){
			goto end;
		}
		ret = aw20036_lamps_flowing_switch(aw20036, led_off);
		if (ret < 0){
			goto end;
		}
	}
						...
						const char *buf, size_t len)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw20036 *aw20036 = container_of(led_cdev, struct aw20036, cdev);
	int led0=0, led1=0, led2=0, led3 =0, led4=0, led5 =0, led6 =0, lamps_effect =0, lamps_status = 0;
	...
	if (sscanf(buf, "%d %d %d %d %d %d %d %d %d",
		&led0, &led1, &led2, &led3, &led4, &led5, &led6, &lamps_effect, &lamps_status) == 9) {

		LOG_INFO("%s set brightness led0=%d %d %d %d %d %d %d %d %d\n", __func__, led0, led1, led2, led3, led4, led5, led6, lamps_effect, lamps_status);

		if (led0 || led1 || led2 || led3 || led4 || led5 || led6 || lamps_effect) {
		...
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED0_REG_FADE, led0);
			if (ret < 0) {
				return ret;
			}
		}
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED1_REG_FADE, led1);
			if (ret < 0) {
				return ret;
			}
		}
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED2_REG_FADE, led2);
			if (ret < 0) {
				return ret;
			}
		}
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED3_REG_FADE, led3);
			if (ret < 0){
				return ret;
			}
		}
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED4_REG_FADE, led4);
			if (ret < 0){
				return ret;
			}
		}
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED5_REG_FADE, led5);
			if (ret < 0) {
				return ret;
			}
		}
		if (!aw20036->flowing_work_state) {
			ret = aw20036_i2c_write(aw20036, LED6_REG_FADE, led6);
			if (ret < 0){
				return ret;
			}
		}

		if (lamps_effect && !led0 && !aw20036->flowing_work_state) {
			schedule_work(&aw20036->lamps_flowing_work);
		}
		...
		factory_led5 = led5;
		factory_led6 = led6;
		factory_led7 = lamps_effect;
		factory_led8 = lamps_status;
				...
				struct device_attribute *attr, char *buf)
{
	...
	LOG_INFO("%s aw20036->always_on:%d\n", __func__, aw20036->always_on);
				...
				struct device_attribute *attr, char *buf)
{
	...
	LOG_INFO("%s aw20036->handset_mode:%d\n", __func__, aw20036->handset_mode);
...
static ssize_t aw20036_video_mode_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw20036 *aw20036 = container_of(led_cdev, struct aw20036, cdev);

	LOG_INFO("%s aw20036->video_mode:%d\n", __func__, aw20036->video_mode);

	return sprintf(buf, "%d\n", aw20036->video_mode);
}

static ssize_t aw20036_video_mode_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	...
	LOG_INFO("%s video_mode:%d -> %d (op_mode %d)\n", __func__, aw20036->video_mode, val, aw20036->operating_mode);

	if (val && (!aw20036->video_mode)) {
		/*When video mode, use 4kHz*/
		aw20036->video_mode = 1;
		if (aw20036->operating_mode != 0) {
			aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE0);
			aw20036_i2c_write(aw20036, 0x81, 0x57);
			aw20036_i2c_write(aw20036, 0x82, 0x3B);
			aw20036_i2c_write(aw20036, 0x83, 0xA7);
			aw20036_i2c_write(aw20036, 0x84, 0x00);
			aw20036_i2c_write(aw20036, 0x85, 0xA0);
			aw20036_i2c_write(aw20036, 0x86, 0x00);
		}
	} else if (!val && (aw20036->video_mode)) {
		 /*When normal mode, use 555Hz*/
		aw20036->video_mode = 0;
		if (aw20036->operating_mode != 0) {
			aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE0);
			aw20036_i2c_write(aw20036, 0x81, 0x57);
			aw20036_i2c_write(aw20036, 0x82, 0x3B);
...
static DEVICE_ATTR(video_mode, 0664, aw20036_video_mode_show, aw20036_video_mode_store);
...
static void aw20036_vip_notification_work(struct work_struct *work)
{
	...
	if (aw20036->vip_notification_id > (sizeof(breath_effect)/sizeof(breath_effect[0])-1)) {
		LOG_INFO("%s not support this vip notification", __func__);
		if (aw20036->vip_notification == 0) {
...
{
	struct aw20036 *aw20036 = container_of(work, struct aw20036, leds_effect_work);
	uint32_t retry = 0;
	unsigned char brightness[ALL_CHANNEL] = {0};
	...
	LOG_INFO("effect working status IdxBuf@%d status 0x%X length %d sec_num %d stream_mode %d\n",
		aw20036->curr_buf->bit, aw20036->curr_buf->status, aw20036->curr_buf->length,
		aw20036->sec_num, aw20036->stream_mode);

	if (aw20036->start_buf->status == MMAP_BUF_DATA_FINISHED) {
		flag = 1;
		aw20036->start_buf->status = MMAP_BUF_DATA_VALID;
		LOG_INFO("effect working IdxBuf@%d Ignore the MMAP_BUF_DATA_FINISHED judgment once!", aw20036->curr_buf->bit);
	}
	do {
		if (aw20036->curr_buf->status == MMAP_BUF_DATA_VALID) {
			...
			if (aw20036->sec_num == VALID_CHANNEL) {
				LOG_INFO("effect working IdxBuf@%d length %d br %d video %d\n",
					aw20036->curr_buf->bit, aw20036->curr_buf->length, aw20036->curr_buf->brightness, video_indicator_val);
				for (i = 0; i < aw20036->curr_buf->length; i += VALID_CHANNEL) {
					start = ktime_get();
					...
					if (aw20036->stream_mode == 0) {
						LOG_INFO("effect working break after next frame stream_mode@%d\n", aw20036->stream_mode);
						break;
					}
				}
				if ((aw20036->curr_buf->length < 468) || (aw20036->stream_mode == 0)){
					LOG_INFO("effect working length %d stream_mode %d\n", aw20036->curr_buf->length, aw20036->stream_mode);
					break;
				}
			}
			...
			LOG_INFO("effect working break IdxBuf@%d status @finish\n", aw20036->curr_buf->bit);
			break;
		} else {
			if (aw20036->stream_mode == 0) {
			...
			LOG_INFO("effect working effect working IdxBuf@%d status 0x%X waiting\n", aw20036->curr_buf->bit, aw20036->curr_buf->status);
			msleep(1);
		}
		if (aw20036->stream_mode == 0) {
			LOG_INFO("effect working break stream_mode@%d retry@%d\n", aw20036->stream_mode, retry);
			break;
		}
	} while(retry++ < 30);
	...
	*/
	...
	ev_happen = 1;
	ev_code = '1';
	LOG_INFO("effect working ev_happen:%d ev_code:%c\n", ev_happen, ev_code);
...
long aw20036_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case LED_STRIPS_STREAM_MODE:
			...
			LOG_INFO("%s stream_mode @1 sec_num:%d to start\n", __func__, aw20036->sec_num);
...
static void aw20036_i2c_remove(struct i2c_client *i2c)
{
	...
	LOG_INFO("%s enter\n", __func__);
	cancel_work_sync(&aw20036->vip_notification_work);
	cancel_work_sync(&aw20036->leds_effect_work);
...
static int __init aw20036_i2c_init(void)
{
	...
	LOG_INFO("aw20036 driver version %s\n", AW20036_DRIVER_VERSION);

	ret = i2c_add_driver(&aw20036_i2c_driver);
	if (ret) {
		LOG_ERR("fail to add aw20036 device into i2c\n");
		return ret;
	}
