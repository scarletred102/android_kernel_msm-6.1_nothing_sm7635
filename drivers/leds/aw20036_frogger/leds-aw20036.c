...
static int aw20036_led_init(struct aw20036 *aw20036)
{
	...
	*/
	...
	aw20036_i2c_write(aw20036, 0x83, 0x00);
	aw20036_i2c_write(aw20036, 0x84, 0x01);
	aw20036_i2c_write(aw20036, 0x85, 0xFF);
...
static void aw20036_leds_effect_work(struct work_struct *work)
{
	...
	pr_info("%s status IdxBuf@%d status 0x%x length %d sec_num %d stream_mode %d\n", __func__,
	...
	do {
		if (aw20036->curr_buf->status == MMAP_BUF_DATA_VALID) {
			...
			if (aw20036->sec_num == VALID_CHANNEL) {
				pr_info("effect working IdxBuf@%d length %d br %d\n",
					aw20036->curr_buf->bit, aw20036->curr_buf->length, aw20036->curr_buf->brightness);
				for (i = 0; i < aw20036->curr_buf->length; i += VALID_CHANNEL) {
					...
					if (aw20036->stream_mode == 0) {
						pr_info("%s stream_mode 0 break 1\n", __func__);
						break;
					}
					aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE2);
					aw20036_i2c_write_block(aw20036, 0x00, ALL_CHANNEL, brightness); /*led(0-35)*/
					if (aw20036->stream_mode == 0) {
						pr_info("%s stream_mode 0 break 2\n", __func__);
						break;
					}
					...
					if (aw20036->stream_mode == 0) {
						pr_info("%s stream_mode 0 break 3\n", __func__);
						break;
					}
				}
				if ((aw20036->curr_buf->length < 468) || (aw20036->stream_mode == 0)){
					pr_info("%s length %d stream_mode %d\n", __func__, aw20036->curr_buf->length, aw20036->stream_mode);
					break;
				}
			}
			...
			pr_info("break IdxBuf@%d status @finish\n", aw20036->curr_buf->bit);
			break;
		} else {
			if (aw20036->stream_mode == 0) {
				pr_info("%s stream_mode 0 break 4\n", __func__);
				break;
			}
			msleep(1);
		}
		if (aw20036->stream_mode == 0) {
			pr_info("%s stream_mode 0 break 5\n", __func__);
			break;
		}
	} while( retry++ < 30);
