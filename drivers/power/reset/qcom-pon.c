...
struct pm8916_pon {
	...
	/* Catch Dump During S2idle. System wakeup when pwrkey press, use Resin instead.Also disable resin hw interrupt*/
				...
				const char *buf, size_t size)
{
	...
	if (pon->force_resin_warm_in_s2idle == 1) {
		...
		//Set Resin Timer and Type, set to 0ms
		rc += regmap_write(pon->regmap, 0x844, 0x0);
		rc += regmap_write(pon->regmap, 0x845, 0x0);
		rc += regmap_write(pon->regmap, 0x846, 0x1);
		//Enable S2 Reset
		rc += regmap_write(pon->regmap, 0x847, 0x80);
		if (rc)
