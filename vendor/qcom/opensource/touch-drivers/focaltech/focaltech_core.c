    ...
    u8 id_h, u8 id_l, bool fw_valid)
{
    ...
    if (id_l == 0x72 || id_l == 0xb3) {
        ...
        for (i = 0; i < ctype_entries; i++) {
            if (VALID == fw_valid) {
                if (((id_h == ctype_3683g[i].chip_idh) && (id_l == ctype_3683g[i].chip_idl))
                    || (!fts_match_cid(ts_data, ctype_3683g[i].type, id_h, id_l, 0)))
                    break;
            } else {
                if (((id_h == ctype_3683g[i].rom_idh) && (id_l == ctype_3683g[i].rom_idl))
                    || ((id_h == ctype_3683g[i].pb_idh) && (id_l == ctype_3683g[i].pb_idl))
                    || ((id_h == ctype_3683g[i].bl_idh) && (id_l == ctype_3683g[i].bl_idl))) {
                    break;
                }
            }
        }
        ...
        for (i = 0; i < ctype_entries; i++) {
            if (VALID == fw_valid) {
                if (((id_h == ctype_3681[i].chip_idh) && (id_l == ctype_3681[i].chip_idl))
                    || (!fts_match_cid(ts_data, ctype_3681[i].type, id_h, id_l, 0)))
                    break;
            } else {
                if (((id_h == ctype_3681[i].rom_idh) && (id_l == ctype_3681[i].rom_idl))
                    || ((id_h == ctype_3681[i].pb_idh) && (id_l == ctype_3681[i].pb_idl))
                    || ((id_h == ctype_3681[i].bl_idh) && (id_l == ctype_3681[i].bl_idl))) {
                    break;
                }
            }
        }
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            if (ret) {
                FTS_ERROR("enable vci regulator failed,ret=%d", ret);
            }
        ...
        if (!ts_data->power_disabled) {
            ...
            if (!IS_ERR_OR_NULL(ts_data->vci)) {
                ...
                if (ret) {
                    FTS_ERROR("disable vci regulator failed,ret=%d", ret);
                }
            }
...
static int fts_power_source_exit(struct fts_ts_data *ts_data)
{
    ...
    if (!IS_ERR_OR_NULL(ts_data->vddio)) {
        if (regulator_count_voltages(ts_data->vddio) > 0)
            regulator_set_voltage(ts_data->vddio, 0, FTS_IOVCC_VTG_MAX_UV);
        regulator_put(ts_data->vddio);
    }
