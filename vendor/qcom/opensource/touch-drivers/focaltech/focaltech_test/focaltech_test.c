...
#if IS_ENABLED(CONFIG_NOTHING_IS_FROGGER)
static int fts_test_proc_show(struct seq_file *s, void *unused)
{
    ...
    memset(fwname, 0, sizeof(fwname));
    if (strstr(fts_data->vendor_name, "boe")) {
        snprintf(fwname, FILE_NAME_LENGTH, "%s", FTS_INI_FILE_NAME_BOE);
    } else if (strstr(fts_data->vendor_name, "vxn")) {
...
static ssize_t fts_test_proc_store(
...
//    .owner  = THIS_MODULE,
    ...
    .proc_write  = fts_test_proc_store,
    .proc_read = seq_read,
