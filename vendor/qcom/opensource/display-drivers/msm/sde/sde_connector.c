...
static unsigned int display_panel_id = 0x00;
#endif
...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if (c_conn) {
		queue_work(system_highpri_wq, &c_conn->set_brightness_work);
		return 0;
	}
...
static ssize_t panel_id1_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	panel_id = (display_panel_id >> 16) & 0xFF;

	if (0x00 == panel_id) {
		nt_rx_cmd(sde_conn, read_id_cmd, code_len);
		panel_id = sde_conn->cmd_rx_buf[0];
		display_panel_id |= (panel_id << 16);
	}
...
static ssize_t panel_id2_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	panel_id = (display_panel_id >> 8) & 0xFF;

	if (0x00 == panel_id) {
		nt_rx_cmd(sde_conn, read_id_cmd, code_len);
		panel_id = sde_conn->cmd_rx_buf[0];
		display_panel_id |= (panel_id << 8);
	}
...
static ssize_t panel_id3_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	panel_id = display_panel_id & 0xFF;

	if (0x00 == panel_id) {
		nt_rx_cmd(sde_conn, read_id_cmd, code_len);
		panel_id = sde_conn->cmd_rx_buf[0];
		display_panel_id |= panel_id;
	}
