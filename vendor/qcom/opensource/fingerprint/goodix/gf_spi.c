...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	pm_wakeup_ws_event(fp_ws,msecs_to_jiffies(WAKELOCK_HOLD_TIME),1);
	sendnlmsg(GF_NETLINK_IRQ, NULL);
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		...
		if (gf_dev->users == 1) {
			...
			if(status)
				pr_err("goodix fp parse drm-panel fail\n");
			...
			if(gf_dev->active_panel){
				gf_dev->cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
					...
					gf_dev->active_panel,gf_dev->notifier, gf_dev);
				if(!gf_dev->cookie)
					pr_err("goodix fp drm_panel_notifier_register fail\n");
			}
                                        ...
                                        void *pvt_data)
{
	...
	if (!notification || !notification->notif_data.early_trigger)
		return ;
	pr_info("[info] %s go to the goodix_fb_state_chg_callback type = %d\n",
			...
			unsigned long event, void *data)
{
	...
	gf_dev = container_of(self, struct gf_dev, tp_notifier);
	switch (event) {
		case TOUCHPANEL_EVENT_NOTIFIER_EVENT_FINGER_UP:
			...
			pr_info("[%s] : TOUCHPANEL_FPEVENT_UP,event=%ld,x=%d,y=%d\n", \
								__func__, event, coordinate->x, coordinate->y);
			sendnlmsg(GF_NETLINK_FINGER_UP, coordinate);
			break;
		case TOUCHPANEL_EVENT_NOTIFIER_EVENT_FINGER_DOWN:
			/*message send format. data[0]:up/or down; data[1]:x coordinate; data[2]:y coordinate.*/
			pr_info("[%s] : TOUCHPANEL_FPEVENT_DOWN,event=%ld,x=%d,y=%d\n", \
								__func__, event, coordinate->x, coordinate->y);
			sendnlmsg(GF_NETLINK_FINGER_DOWN, coordinate);
			break;
		default:
			pr_info("[%s] : Ignore event, event=%ld\n", __func__, event);
		break;
	}
...
static int nt_dsi_notifier_callback(struct notifier_block *self,
			unsigned long event, void *data)
{
	struct gf_dev *gf_dev = NULL;
	int retval = 0;
	...
	gf_dev = container_of(self, struct gf_dev, dsi_notifier);
	switch (event) {
		case FP_NOTIFIER_EVENT_UI:
			/*both hbm_status=1 ui_status=1, then UI Ready. otherwise UI disappear.*/
			pr_info("[%s] :FP_NOTIFIER_EVENT_UI hbm_status=%d,ui_status=%d\n", __func__, fp_event->hbm_status, fp_event->ui_status);
			if((1 == fp_event->hbm_status) && (1 == fp_event->ui_status)) {
				sendnlmsg(GF_NETLINK_UI_READY, fp_event);
				pr_info("[%s]:FP_NOTIFIER_EVENT_UI FP_UI_READY #^_^#", __func__);
			} else {
				sendnlmsg(GF_NETLINK_UI_DISAPPEAR, fp_event);
				pr_info("[%s]:FP_NOTIFIER_EVENT_UI FP_UI_DISAPPEAR #@_@#", __func__);
			}
			break;
		default:
			pr_info("[%s] : Ignore event, event=%ld\n", __func__, event);
...
#endif
{
	...
	gf_dev->vdd_max_uv = -EINVAL;
	gf_dev->vdd_min_uv = -EINVAL;
	gf_dev->vdd = NULL;
	gf_dev->cookie = NULL;
	gf_dev->device_available = 0;
	gf_dev->fb_black = 0;
	...
	gf_dev->notifier = goodix_panel_state_chg_callback;
	/*fb_register_client(&gf_dev->notifier);*/

	gf_dev->tp_notifier.notifier_call = nt_tp_notifier_callback;
	status = touchpanel_event_register_client(&gf_dev->tp_notifier);
	if (status < 0) {
		pr_err("%s, touchpanel_event_register_notifier fail: %d\n", __func__, status);
	}

	gf_dev->dsi_notifier.notifier_call = nt_dsi_notifier_callback;
	status = nt_display_event_register_notifier(&gf_dev->dsi_notifier);
	if (status < 0) {
		pr_err("%s, nt_dsi_register_notifier fail: %d\n", __func__, status);
	}

	fp_ws = wakeup_source_register(&gf_dev->spi->dev,"fp_ws");
...
#endif
{
	...
	wakeup_source_unregister(fp_ws);
//	fb_unregister_client(&gf_dev->notifier);
	...
	touchpanel_event_unregister_client(&gf_dev->tp_notifier);
	nt_display_event_unregister_notifier(&gf_dev->dsi_notifier);
...
static int __init gf_init(void)
{
	...
	pr_err("djy enter register spi driver");
