...
int __attribute__((weak)) send_tfa_cal_apr(void *buf, int cmd_size, bool bRead)
{
	(void)buf;
	(void)cmd_size;
	(void)bRead;
    pr_err("[%s]you need to define this func in other module!\n", __FUNCTION__);
	return 0;
}

int __attribute__((weak))  send_tfa_cal_in_band(void *buf, int cmd_size)
{
	(void)buf;
	(void)cmd_size;
    pr_err("[%s]you need to define this func in other module!\n", __FUNCTION__);
...
int __attribute__((weak)) mtk_spk_send_ipi_buf_to_dsp(void *buffer, uint32_t size)
{
	(void)buffer;
	(void)size;
    pr_err("[%s]you need to define this func in other module!\n", __FUNCTION__);
...
int __attribute__((weak))  mtk_spk_recv_ipi_buf_from_dsp(int8_t *buffer, int16_t size, uint32_t *data_len)
{
	(void)buffer;
	(void)size;
    (void)data_len;
...
//		kfree(msg_file);
	...
	if (tfa98xx->tfa->is_probus_device) {
		if (tfa98xx_device_count > 2 && (tfa98xx->tfa->dev_idx >=2))
...
static int tfa98xx_info_stop_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER; // TYPE_BOOLEAN meet some errors.
	mutex_lock(&tfa98xx_mutex);
	uinfo->count = tfa98xx_device_count;
	mutex_unlock(&tfa98xx_mutex);
	uinfo->value.integer.min = 0;
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	nr_controls += 2; /* for casa,  pcm id, miid */
	...
	tfa98xx_write_dsp(struct tfa_device *tfa, int num_bytes, const char *command_buffer)
{
    ...
    if ( tfa->dev_idx == 2 || tfa->dev_idx==3 )
        *(uint8_t*)(&buffer[0]) |= ((tfa->dev_idx/2)<<4) & 0xf0;
    pr_err("%s-inst is 0x%x\n", __func__, buffer[0]);
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	if (TFA_GET_BF(tfa98xx_pri->tfa, MTPEX) == 1)
	{
		...
		cal_bytes[4] = (uint8_t)((dsp_cal_value >> 16) & 0xff);
		cal_bytes[5] = (uint8_t)((dsp_cal_value >> 8) & 0xff);
		cal_bytes[6] = (uint8_t)(dsp_cal_value & 0xff);
        cal_bytes[0] += 1;
		/* We have to copy it for both channels. Even when mono! */
		cal_bytes[7] = cal_bytes[4];
		cal_bytes[8] = cal_bytes[5];
		cal_bytes[9] = cal_bytes[6];

        pr_info("Device0 0x%x cal value is 0x%d,value:%d\n", tfa98xx_pri->i2c->addr, dsp_cal_value,value);
	}
		...
		TFA_GET_BF(tfa98xx_sec->tfa, MTPEX) == 1)
	{
		...
		cal_bytes[7] = (uint8_t)((dsp_cal_value >> 16) & 0xff);
		cal_bytes[8] = (uint8_t)((dsp_cal_value >> 8) & 0xff);
		cal_bytes[9] = (uint8_t)(dsp_cal_value & 0xff);
        cal_bytes[0] += 1;

        pr_info("Device1 0x%x cal value is 0x%d,value:%d\n", tfa98xx_sec->i2c->addr, dsp_cal_value,value);
	}

    /* we will send it to host DSP algorithm once calibraion value loaded from all device. */
    if (tfa98xx_device_count == cal_bytes[0]) {
        cal_bytes[1] = 0x00;
        cal_bytes[2] = 0x81;
        cal_bytes[3] = 0x05;
		...
		tfa98xx_send_data_to_dsp(&cal_bytes[1], sizeof(cal_bytes) - 1);
        msleep(10);
        ...
        if (cal_bytes[0] > tfa98xx_device_count)
            cal_bytes[0] = 0;
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (1 == tfa98xx_device_count) {
			if (tfa98xx->tfa->dev_idx == 0)
				...
				memcpy(&bytes_inst0[4], &bytes_inst0[7], sizeof(char)*3);
		}
	}
	...
	/* we will send it to host DSP algorithm once calibraion value loaded from all device. */
	if (speaker_count == bytes_inst0[0]) {
		bytes_inst0[1] = 0x00;
		bytes_inst0[2] = 0x81;
		bytes_inst0[3] = 0x05;
	...
	if ((ret==0) && (speaker_count == bytes_inst1[0])) {
		bytes_inst1[1] = 0x10;
		bytes_inst1[2] = 0x81;
		bytes_inst1[3] = 0x05;
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
#endif
	pr_info("%s[0x%x]: state: %d, stream : %d\n", __func__, tfa98xx->i2c->addr, mute, stream);
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	{
        ...
        .symmetric_rate = 1,                    //modify for kernel6.1
#else
        .symmetric_rates = 1,
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,12,0)
        .symmetric_sample_bits = 1,             //modify for kernel6.1
#else
...
#else
static int tfa98xx_i2c_remove(struct i2c_client *i2c)           //modify for kernel6.1
#endif
{
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
