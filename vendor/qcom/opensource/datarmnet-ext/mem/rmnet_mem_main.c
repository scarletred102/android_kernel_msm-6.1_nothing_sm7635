...
GFP_ATOMIC|__GFP_NOMEMALLOC):gfp_mask,order);if(page){if(DATARMNET8224a106d8){
rmnet_mem_add_page(page,order);page_ref_inc(page);}if(pageorder){*pageorder=
order;}}}else{if(DATARMNET8224a106d8){page=__dev_alloc_pages((
DATARMNET8224a106d8)?(GFP_ATOMIC|__GFP_NOMEMALLOC):gfp_mask,order);if(page){
rmnet_mem_add_page(page,order);page_ref_inc(page);}if(pageorder){*pageorder=
order;}}}}if(DATARMNET8224a106d8)spin_unlock_irqrestore(&rmnet_mem_lock,flags);
if(pageorder&&code&&page){if(*pageorder==order)*code=RMNET_MEM_SUCCESS;else if(*
pageorder>order)*code=RMNET_MEM_UPGRADE;else if(*pageorder<order)*code=
RMNET_MEM_DOWNGRADE;}else if(pageorder&&code){*code=RMNET_MEM_FAIL;*pageorder=
(0xd2d+202-0xdf7);}return page;}EXPORT_SYMBOL(rmnet_mem_get_pages_entry);void 
...
__dev_alloc_pages(GFP_ATOMIC|__GFP_NOMEMALLOC,pageorder);if(!newpage){continue;}
