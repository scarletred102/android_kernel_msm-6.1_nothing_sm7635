 ...
 * programmer because of the combined behavior of the obfuscation tool and the
 * C preprocessor. Because the tool scrambles numbers into (a + b - c), you
 * can't simply make macros for each configuratble knob and append them to some
 ...
 * by hand, and ensure that the name is placed in the list of exception symbols
 * when running the obfuscator tool. Sorry about that...
 *
 ...
 * Rememeber to add the name to the exception lists for obfuscation, otherwise
 * it sure won't be called what you think it will ;)
 */
