...
int rmnet_shs_get_suggested_cpu(struct rmnet_shs_skbn_s *node)
{
	...
	/* Return same perf core unless moving to gold from silver*/
		...
		cpu = rmnet_shs_get_core_prio_flow(PERF_MASK & ~rmnet_shs_cfg.ban_mask & ~rmnet_shs_reserve_mask &
						   node->custom_map);
		...
		if ((1 << node->map_cpu) & (rmnet_shs_cfg.ban_mask | rmnet_shs_reserve_mask)) {
			/* if flow is on a banned cpu move it to most avaible cpu on same cluster
			 */
			cluster = (PERF_MASK & (1 << cpu))? PERF_MASK : NONPERF_MASK;
            /* If flow lands on banned cpu move up a cluster */
            if (!(cluster & ~rmnet_shs_cfg.ban_mask & ~rmnet_shs_reserve_mask &  node->custom_map))
                cluster = PERF_MASK;

			cpu = rmnet_shs_get_core_prio_flow(cluster &
				                           ~rmnet_shs_cfg.ban_mask & ~rmnet_shs_reserve_mask &  node->custom_map);
		...
		if ((!node->phy && cur_cpu_qhead >= (node_qhead + node->qhead_offset))  ||
		   ...
		   /* Phy flows old cpu will be empty but OOO is still posible, focus on */
			...
			if (likely(rmnet_shs_switch_cores)) {
				...
				} else if ((1 << cpu_num ) & rmnet_shs_reserve_mask) {
...
void rmnet_shs_flush_core(u8 cpu_num)
{
	 ...
	 */
	if (cpu_num == rmnet_shs_cfg.phy_old_cpu && rcu_dereference(rmnet_shs_switch)) {
...
{
	struct rmnet_shs_cpu_topology_s rmnet_shs_cpuinfo[MAX_CPUS];
	int i = 0, cpu_num = 0;
	int min_cap = 0, min_cap_cpu_idx = 0, cpu = 0, siblings = 0;

	memset(rmnet_shs_cpuinfo, 0, MAX_CPUS * sizeof(struct rmnet_shs_cpu_topology_s));

	for_each_cpu(cpu, cpu_possible_mask) {
		siblings = 0;
		rmnet_shs_cpuinfo[cpu].core = cpu;
		rmnet_shs_cpuinfo[cpu].cid = topology_cluster_id(cpu);

		for_each_cpu(siblings, topology_cluster_cpumask(cpu)) {
			rmnet_shs_cpuinfo[cpu].cmask |= 1 << siblings;
		}

		rmnet_shs_cpuinfo[cpu].capacity = arch_scale_cpu_capacity(cpu);
		rm_err("SHS_CPU_LOG: cpu: %d cid: %d cmask: %d capacity: %d\n",
			rmnet_shs_cpuinfo[cpu].core,
			rmnet_shs_cpuinfo[cpu].cid,
			rmnet_shs_cpuinfo[cpu].cmask,
			rmnet_shs_cpuinfo[cpu].capacity);
	}

	/*Finding lowest capacity CPU and using its mask as non_perf mask*/
	min_cap = rmnet_shs_cpuinfo[0].capacity;
	min_cap_cpu_idx = 0;
	for(i = 0; i < MAX_CPUS; i++) {
		if(rmnet_shs_cpuinfo[i].capacity < min_cap) {
			min_cap = rmnet_shs_cpuinfo[i].capacity;
			min_cap_cpu_idx = i;
		}
	}

	rmnet_shs_cfg.non_perf_mask = rmnet_shs_cpuinfo[min_cap_cpu_idx].cmask;
	rmnet_shs_cfg.max_s_cores = hweight_long(rmnet_shs_cfg.non_perf_mask);
	rmnet_shs_cfg.perf_mask = (0xFF ^ rmnet_shs_cfg.non_perf_mask);

	for (cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
		if ((1 << cpu_num) & rmnet_shs_cfg.non_perf_mask) {
			 rmnet_shs_cpu_rx_min_pps_thresh[cpu_num] = RMNET_SHS_UDP_PPS_LPWR_CPU_LTHRESH;
			 rmnet_shs_cpu_rx_max_pps_thresh[cpu_num] = RMNET_SHS_UDP_PPS_LPWR_CPU_UTHRESH;
		}
		else {
			rmnet_shs_cpu_rx_min_pps_thresh[cpu_num] = RMNET_SHS_UDP_PPS_PERF_CPU_LTHRESH;
			rmnet_shs_cpu_rx_max_pps_thresh[cpu_num] = RMNET_SHS_UDP_PPS_PERF_CPU_UTHRESH;
		}
	}

	rm_err("SHS_CPU_LOG: max_s_cores: %d non_perf_mask: %d perf_mask: %d\n",
		rmnet_shs_cfg.max_s_cores,
...
void rmnet_shs_init(struct net_device *dev, struct net_device *vnd)
{
	...
	rmnet_shs_cfg.ban_mask = 0x2;
	rmnet_shs_cfg.phy_tcpu = 1;
	rmnet_shs_cfg.phy_old_cpu = 1;
	rmnet_shs_cfg.phy_acpu = 1;

	rmnet_shs_update_cpumask();
	...
	do {
		hash_for_each_possible_safe(RMNET_SHS_HT, node_p, tmp, list,
			...
			if (node_p->phy) {
				...
				if (!rcu_dereference(rmnet_shs_switch)) {
		...
		cluster_mask =  (NONPERF_MASK & ~rmnet_shs_cfg.ban_mask & ~rmnet_shs_reserve_mask & rmnet_shs_cfg.map_mask) ?
