...
{
#if IS_ENABLED(CONFIG_SCHED_WALT)
	cpumask_t rmask = { CPU_BITS_NONE };
	int j;
	unsigned int dest_mask = 0;

	if (walt_get_cpus_in_state1(&rmask) < 0)
		return;
	...
	{
		if(((rmnet_shs_cfg.perf_mask & (1 << TITANIUM_CPU1)) &&
		     (rmnet_shs_cfg.perf_mask & (1 << TITANIUM_CPU2)))) {
			/*Titanium is running at Silver freq*/
			rmnet_shs_cfg.non_perf_mask |= 1 << TITANIUM_CPU1;
			rmnet_shs_cfg.non_perf_mask |= 1 << TITANIUM_CPU2;
		...
		if(((rmnet_shs_cfg.non_perf_mask & (1 << TITANIUM_CPU1)) &&
		     (rmnet_shs_cfg.non_perf_mask & (1 << TITANIUM_CPU2)))) {
			/*Titanium is running at Gold freq*/
			rmnet_shs_cfg.perf_mask |= 1 << TITANIUM_CPU1;
			rmnet_shs_cfg.perf_mask |= 1 << TITANIUM_CPU2;
			rmnet_shs_cfg.non_perf_mask = (0xFF ^ rmnet_shs_cfg.perf_mask);
			rmnet_shs_cfg.max_s_cores = hweight_long(rmnet_shs_cfg.non_perf_mask);
...
static void rmnet_update_reserve_mask(void)
{
	...
	rmnet_shs_reserve_mask = update_mask;

	/* Move to Perf core if silver phy is taken, move to another Perf core if perf phy is taken */
	if ((1 << rmnet_shs_cfg.phy_acpu) &  rmnet_shs_reserve_mask) {
		new_cpu = rmnet_shs_wq_get_least_utilized_core(rmnet_shs_cfg.map_mask & PERF_MASK &
							       ~rmnet_shs_cfg.ban_mask & ~rmnet_shs_reserve_mask);
		if (new_cpu > 0) {
			new_phy = new_cpu;
	...
	list_for_each_entry(ep, &rmnet_shs_wq_ep_tbl, ep_list_id) {
		if (node_p->dev == ep->ep) {
			...
			if (hstat_p->skb_tport_proto == IPPROTO_TCP) {
				rm_err("SHS_TCP: adding TCP bps %lu to ep_total %lu ep name %s",
...
void rmnet_shs_wq_update_hash_stats(struct rmnet_shs_wq_hstat_s *hstats_p)
{
	...
	rm_err1("SHS_SEGS: hash 0x%x coal skb = %lu | last coal skb = %lu | rx skb = %lu | last rx skb %lu",
	       hstats_p->hash,
	...
	if (coal_skb_diff > 0) {
		...
		rm_err1("SHS_SEGS: avg segs = %lu skb_diff = %lu coal_skb_diff = %lu",
		       hstats_p->avg_segs, skb_diff, coal_skb_diff);
	...
	rm_err1("SHS_HW_COAL: hw coal bytes = %lu hw coal bufsize = %lu",
		node_p->hw_coal_bytes, node_p->hw_coal_bufsize);
	rm_err1("SHS_HW_COAL: LAST: hw coal bytes = %lu hw coal bufsize = %lu",
		hstats_p->last_hw_coal_bytes, hstats_p->last_hw_coal_bufsize);
	rm_err1("SHS_HW_COAL: hw coal bytes diff = %lu hw coal bufsize diff = %lu",
...
int rmnet_shs_wq_get_least_utilized_core(u16 core_msk)
{
	...
	for (cpu_num = MAX_CPUS-1; cpu_num >= 0; cpu_num--) {
		...
		is_cpu_in_msk = ((1 << cpu_num) & core_msk) && cpu_online(cpu_num) && !is_reserved(cpu_num);
		if (!is_cpu_in_msk)
			continue;
					...
					struct rmnet_shs_wq_ep_s *ep)
{
	...
	for (cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
		...
		is_core_in_msk = ((1 << cpu_num) & (ep->rps_config_msk & ~rmnet_shs_cfg.ban_mask) & ~rmnet_shs_reserve_mask);
				       ...
				       struct rmnet_shs_wq_ep_s *ep)
{
	...
	cpu_in_rps_mask = (1 << dest_cpu) & ep->rps_config_msk & ~rmnet_shs_cfg.ban_mask & ~rmnet_shs_reserve_mask;
...

{
	...
	for (cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
		...
		if ((avg_pps > pps_uthresh) ||
		    ((1 << cpu_num) & (rmnet_shs_cfg.ban_mask | rmnet_shs_reserve_mask)) ||
		    !cpu_online(cpu_num) ||
...
void rmnet_shs_wq_cleanup_hash_tbl(u8 force_clean)
{
	...
	list_for_each_safe(ptr, next, &rmnet_shs_wq_hstat_tbl) {
		...
		if ((rmnet_shs_wq_time_check(tns2s, rmnet_shs_cfg.num_flows) &&
		    ((node_p->phy && !rcu_dereference(rmnet_shs_switch)) || !node_p->phy)) ||
		    force_clean) {
			trace_rmnet_shs_wq_low(RMNET_SHS_WQ_FLOW_STATS,
...
void rmnet_shs_wq_pause(void)
{
	 ...
	 */
	rcu_read_lock();
	if (((1 << rmnet_shs_cfg.phy_acpu) & PERF_MASK) && ((1 << 1) & ~rmnet_shs_reserve_mask)) {
